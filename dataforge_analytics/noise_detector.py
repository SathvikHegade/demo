...
def detect_outliers(df: pd.DataFrame, column: str, method: str = "iqr") -> OutlierReport:
    ...
    if column not in df.columns or not pd.api.types.is_numeric_dtype(df[column]):
        return OutlierReport(column_name=column, outlier_count=0, outlier_indices=[], outlier_percentage=0.0, method_used=method, bounds={})
...
def detect_datatype_noise(df: pd.DataFrame) -> DataTypeNoiseReport:
    """Detect type inconsistencies like numeric as strings."""
    affected = []
    format_issues = {}
    type_mismatch = 0
...
def detect_value_noise(df: pd.DataFrame) -> ValueNoiseReport:
    ...
    for col in df.columns:
        if pd.api.types.is_object_dtype(df[col]) or pd.api.types.is_string_dtype(df[col]):
            ...
            col_groups = {}
            if 0 < len(uniques) < min(100, len(s) / 2):  # Apply fuzzy match to categorical-like strings
               for u in uniques:
                   matches = []
            ...
            if len(sentinels) > 0 or len(whitespace_issues) > 0 or col_groups:
                if col not in affected: affected.append(col)
                if col_groups:
                    merged = {}
